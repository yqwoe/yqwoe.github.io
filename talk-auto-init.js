...
// 配置信息
const config = {
  username: 'yqwoe', // GitHub repository 所有者，可以是个人或者组织。对应Gitalk配置中的owner
  repo: "yqwoe.github.io", // 储存评论issue的github仓库名，仅需要仓库名字即可。对应 Gitalk配置中的repo
  token: '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', // 前面申请的 personal access token
...
const sitemapXmlReader = (file) => {
  try {
    ...
    sitemap.root.children.forEach(function (url) {
      ...
      const id = url.children.find(function (item) {
        return item.name === 'id';
      });
